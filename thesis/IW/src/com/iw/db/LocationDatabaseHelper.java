...
public class LocationDatabaseHelper extends SQLiteOpenHelper {
    ...
    // Database Version
    private static final int DATABASE_VERSION = 6; // after adding 3G sig
                                                   // strength

    // Database Name
    private static final String DATABASE_NAME = "droidtn_location";
    ...
    public static final int TABLE_POINTS_NUM_COLUMNS = 9;
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_POINTS_TABLE = "CREATE TABLE " + TABLE_POINTS + "("
                        ...
                        + KEY_TIMESTAMP + " INTEGER," + KEY_WIFI_POWER_LEVELS
                        + " STRING," + KEY_SPEED + " REAL," + KEY_ACCURACY
                        + " REAL," + KEY_SIGNAL_STRENGTH + " INTEGER" + ")";
        db.execSQL(CREATE_POINTS_TABLE);
        String CREATE_PREDICTION_TABLE = "CREATE TABLE " + TABLE_PREDICTION
                        + "(" + KEY_ID + " INTEGER PRIMARY KEY," + KEY_LAT
                        + " REAL," + KEY_LNG + " REAL," + KEY_BEARING
                        + " REAL," + KEY_TIME_TO_WIFI + " INTEGER" + ")";
    ...
    private void addPoint(LocationDataPoint point) {
        if (point.isValid()) {
            SQLiteDatabase db = this.getWritableDatabase();
            ...
            values.put(KEY_SIGNAL_STRENGTH, point.get3GSignalStrength());
    ...
    private void addPredictions(List<Double> lat, List<Double> lng,
                    List<Double> bearing, List<Integer> time_to_wifi) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    private int predict(double lat, double lng, double bearing, double speed,
        ...
        for (int i = 0; i < time_list.size(); i++) {
            double dist = Math.pow(lat - lat_list.get(i), 2)
                            + Math.pow(lng - lng_list.get(i), 2)
                            + Math.pow((bearing - bearing_list.get(i))
                                            * Consts.BEARING_MULTIPLIER, 2);
            if (dist < min_dist) {
                min_dist = dist;
    ...
    private Map<String, String> popFew() {
        ...
        List<String> accuracyList = new ArrayList<String>(
                        2 * Consts.HTTP_BATCH_LIMIT);
        List<String> sigStrengthList = new ArrayList<String>(
                        2 * Consts.HTTP_BATCH_LIMIT);

        // Select All Query
        String selectQuery = "SELECT * FROM " + TABLE_POINTS + " ORDER BY "
                        + KEY_TIMESTAMP + " ASC LIMIT "
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                sigStrengthList.add(cursor.getString(8));
            } while (cursor.moveToNext());
        }
        cursor.close();

        // Delete retrieved points
        db.delete(TABLE_POINTS, KEY_TIMESTAMP + " <= ?",
                        new String[] { Long.toString(greatestTimeStamp) });
        db.close(); // Closing database connection

        data.put(KEY_LAT, StringUtils.implode(latList.toArray(new String[0]),
                        ","));
        data.put(KEY_LNG, StringUtils.implode(lngList.toArray(new String[0]),
                        ","));
        data.put(KEY_BEARING, StringUtils.implode(
                        bearingList.toArray(new String[0]), ","));
        data.put(KEY_TIMESTAMP, StringUtils.implode(
                        timestampList.toArray(new String[0]), ","));
        data.put(KEY_WIFI_POWER_LEVELS, StringUtils.implode(
                        wifipowerlevelList.toArray(new String[0]), ","));
        data.put(KEY_SPEED, StringUtils.implode(
                        speedList.toArray(new String[0]), ","));
        data.put(KEY_ACCURACY, StringUtils.implode(
                        accuracyList.toArray(new String[0]), ","));
        data.put(KEY_SIGNAL_STRENGTH, StringUtils.implode(
                        sigStrengthList.toArray(new String[0]), ","));
    ...
    // ----------------------------
    public synchronized static void addPoint(Context context,
                    LocationDataPoint point) {
        LocationDatabaseHelper db = new LocationDatabaseHelper(context);
        db.addPoint(point);
    }

    public synchronized static void addPredictions(Context context,
                    List<Double> lat, List<Double> lng, List<Double> bearing,
                    List<Integer> time_to_wifi) {
        LocationDatabaseHelper db = new LocationDatabaseHelper(context);
    ...
    public synchronized static int predict(Context context, double lat,
                    double lng, double bearing, double speed, double accuracy,
                    double timestamp) {
        LocationDatabaseHelper db = new LocationDatabaseHelper(context);
