...
param totalUsers integer; 		#Number of users
...
param userTotalSize {i in 1..totalUsers};
...
param streamUser {i in 1..totalPackets, j in 1..totalUsers}; 	#Per-user datastream assignments
#param userTotalSize {i in 1..totalUsers};						#Per-user datastream sizes
param userTotalSize {i in 1..totalUsers}:= for {j in 1..totalUsers} (sum{i in 1..totalPackets} sizeBig[i] * userStream[i][j]);
...
minimize cost:	
	sum{i in 1..totalUsers} (1/userTotalSize[i]) * (sum{j in BigPackets, k in 1..totalSmallPackets[j]} (streamUser[j,i] * (sizeSmall[j,k]+ header[j,k])*channelScheduled[j,k,mobileChannel]));
...
#subject to totalDuration{i in BigPackets, j in 1..totalSmallPackets[i], n in 1..totalChannels}: 	
#	time[i,j]+((sizeSmall[i,j]+header[i,j])/bw[n]) + latency[n] + (channelScheduled[i,j,n]*X*(channelScheduled[i,j,n]-channelScheduled[i,j-1,n])) <= (tmax*(1-channelScheduled[i,j,n]))+end[n];
